...
def main():
    ...
    lc1_path = os.path.join("../data/lc1.csv")  # file path
    ...
    sm_path = os.path.join("../data/sm_20032013.csv")
    ...
    vpd_path = os.path.join("../data/vpd_20032013.csv")
    ...
    evi_20032013 = np.loadtxt("EVI_20032013.csv", delimiter=",").reshape((112, 244, 132))

    # Fire data from 2003-2013
    fire_path = os.path.join("../data/firemon_tot_size.csv")
        ...
        for i in range((np.shape(fire_obs_ini_cate))[0]):
            for j in range((np.shape(fire_obs_ini_cate))[1]):
                if (lc1[i][j] == lctypemat[lc_co]) & (val_new_obs[i][j][0] < below_no_obs).all():
                    ...
                    val_new_obs[i][j] = 1
                elif (lc1[i][j] == lctypemat[lc_co]) & (val_new_obs[i][j][0] >= below_no_obs).all() & (
        ...
        for i in range((np.shape(fire_pred_ini_cate))[0]):
            for j in range((np.shape(fire_pred_ini_cate))[1]):
                if (lc1[i][j] == lctypemat[lc_co]) & (val_new_pred[i][j][0] < below_no_pred).all():
                    ...
                    val_new_pred[i][j] = 1
                elif (lc1[i][j] == lctypemat[lc_co]) & (val_new_pred[i][j][0] >= below_no_pred).all() & (
